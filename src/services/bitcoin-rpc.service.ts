...
export class BitcoinRpcService implements OnModuleInit {
    ...
    private async getAndBroadcastLatestTemplateOnce(
        ...
        // Best-effort cache refresh; must never break template publication.
        // Master-only: it owns Core communication and the Redis cache write,
        // matching the template fetch path itself.
        if (process.env.MASTER === 'true') {
            try {
                await this.refreshMempoolInfo();
            } catch (error) {
                console.warn(`Mempool info cache refresh failed: ${error.message ?? error}`);
            }
        }
    }

    // Piggybacks on the same trigger as the template refresh (new block via
    // longpoll/ZMQ, plus the 60s resetTemplateInterval$ between blocks) rather
    // than running its own polling loop. getmempoolinfo is cheap, and mempool
    // depth changes continuously rather than only per-block, so the template's
    // ~60s cadence (not the ~10min block cadence) is the better fit. Runs after
    // the publication await so it adds zero latency to job fanout.
